...
void tarea2(void* z)
{
    ...
    while (1)
    {
        if (xQueueReceive(buffer, &rcv, pdMS_TO_TICKS(1000)) == pdTRUE) // Si recibe el valor dentro de 1 segundo, muestra en pantalla
        {
            ...
            if (variable == rcv)
            {
                while (1)
                {
                    vTaskDelay(pdMS_TO_TICKS(500)); // Pequeña pausa para evitar rebotes del botón
                     xQueueReceive(buffer, &rcv, pdMS_TO_TICKS(1000)); // Espera hasta que haya un elemento en la cola
                    if (variable == rcv)
                    {
                        break;
                    }
                    ...
                    {
                        printf("Item recibido en ciclo: %lu\n", rcv); // Muestra el valor en pantalla
                        printf("Item recibido en ciclo: %lu\n", variable);
...
void app_main()
{
    ...
    xTaskCreatePinnedToCore(tarea2, "tarea2", 4096, NULL, 4, NULL, 0); // Crea tarea que lee valores de la cola.
}
